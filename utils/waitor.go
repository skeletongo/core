...
func (w *Waitor) Add(name string, delta int) {
	...
	log.Logger.Debugf("(w *Waitor)(%v:%p) Add(%v,%v) counter(%v)", w.name, w, name, delta, cnt)
}
...
func (w *Waitor) Wait(name string) {
	...
	log.Logger.Debugf("(w *Waitor)(%v:%p) Waiter(%v) waiters(%v)", w.name, w, name, cnt)
	for w.counter > 0 {
		dname := <-w.c
		v = atomic.AddInt32(&w.counter, -1)
		cnt = atomic.LoadInt32(&w.counter)
		log.Logger.Debugf("(w *Waitor)(%v:%p) Waiter(%v) after(%v)done! counter(%v)", w.name, w, name, dname, cnt)
	}
}

func (w *Waitor) Done(name string) {
	w.c <- name
	log.Logger.Debugf("(w *Waitor)(%v:%p) Done(%v)!!!", w.name, w, name)
}
